...
minicc 使用 `while True` 无限循环，由模型自己判断任务是否完成。当轮次超过 60 时给出黄色警告，超过 300 时才强制终止。

### 2.3 流式工具执行（P0 优化）

这是 minicc 相比早期版本最重要的性能提升。在旧版本中，工具必须等模型**完全输出完毕**才开始执行——用户盯着旋转图标干等。现在工具在模型**还在输出**时就开始跑了。

**工作原理**：Anthropic API 在流式传输中会逐个发送 `content_block_stop` 事件，表示一个 tool_use 块已完成。StreamingToolExecutor 在收到事件的瞬间就把工具提交到线程池，不等到整个响应结束。

**效果**：假设模型返回 2000 字文本 + 3 个文件读取。旧版本要等 2000 字全部打印完才开始读文件。新版本在文本还在流式输出时，3 个文件已经在后台并行读取了——感知延迟从"串行"变成"流水线"。

### 2.4 并行工具执行

只读工具可以在线程池中并行执行。当模型在一次响应中同时调用多个只读工具，minicc 会用线程池并行执行它们。
...
Engine 被拆分为 4 个模块，各司其职：

| 模块 | 职责 |
|------|------|
| `agent/loop.py` | 精简的 Engine 类 + while True 循环生命周期 |
| `agent/retry.py` | API 重试策略：指数退避 + jitter + 智能错误分类 |
| `agent/tool_executor.py` | StreamingToolExecutor：流式工具提交、并行/串行调度、结果排序 |
| `tools/safety.py` | StuckDetector（重复调用检测）+ StaleReadDetector（重复内容检测） |

拆分后每个模块可以独立测试，Engine 回归纯粹的编排角色。
...
| run_shell | 执行 shell 命令，支持后台运行和自动转入后台 | 否 |
| web_fetch | 获取网页内容并用 prompt 提取信息 | 是 |
...
edit_file 用于精确字符串替换——大多数修改只需要改一两行，不需要重写整个文件。
...
## 六、上下文压缩（参考 Claude Code 五层设计）

当对话历史变长，minicc 不会简单地"消息数到了就压"。它参考了 Claude Code 的多层压缩管线：

### 6.1 选择性微压缩（每轮必跑，零 API 成本）

不是所有工具结果都截断。只压缩 **7 种工具类型**：read_file、list_files、search_files、git_diff、run_shell、write_file、edit_file。Agent、Plan 等工具（将来加入）的结果**永不截断**。

截断前还会先跑一道**结果预算**（`applyToolResultBudget`）：每个工具有 `maxResultSizeChars` 上限（如 read_file 80K、run_shell 30K），超了的结果不塞进消息数组，而是替换为 `<persisted-output>` 占位符和 2KB 预览。

压缩用**时间屏蔽**替代固定数量：最近 30 秒内产生的消息绝对不碰，不管有多少条。这意味着模型密集地读了 10 个文件——全会保留；2 分钟前读了 1 个——可能已经被清了。

### 6.2 自动压缩（token 超阈值触发，有 API 成本）

阈值：`上下文窗口 - 缓冲（13K）`，约为 93.5%。

模型把旧消息生成一份 **6 段式结构化摘要**（User Goal、Files & Project、Commands & Results、Errors & Fixes、Current State、Project Structure）。
...
除了 token 阈值检查，还有**预判检查**：在 API 调用前估算本轮增长（maxOutput + 工具结果 15K），如果会溢出就提前压缩。
...
3. **子 Agent 收到**：对话 transcript + 现有记忆文件列表（含修改时间）+ 记忆目录的读写权限
4. **子 Agent 的策略**：第一轮并行读现有文件，第二轮并行写更新
5. **后台静默运行** — stdout 被重定向，不弹权限框，用户完全无感
...
子 Agent 模式解决了这些问题：主动审阅完整对话，独立判断写什么、改什么、什么过时了。

### 12.4 记忆过期处理：标注而�不删除

参考了 Claude Code 的 `memoryAge.ts` 设计。Claude Code **不自动删除**旧记忆，而是根据文件修改时间（mtime）给模型注入提示：

- 修改时间 **1 天内**的记忆：正常加载，无额外提示
- 修改时间 **超过 1 天**的记忆：注入 `<system-reminder>此记忆是 47 天前的，关于代码行为的描述可能已过时，请验证后再使用</system-reminder>`

这样模型看到旧记忆时会保持警惕——比如 "某文件第 23 行有个 bug" 这种记忆过了 47 天大概率已经不适用了，但 "用户偏好用 pytest" 这种永远不会过时。模型根据提示自行判断该信还是该验证。

另外还有三层容量保护：
- 记忆文件**最多 200 个**（超过的被忽略）
- MEMORY.md 索引**超过 200 行截断**
- 孤儿文件（MEMORY.md 中没引用到的 .md 文件）自动清理

### 12.5 记忆注入

`MEMORY.md` 会被加载到系统提示中。每条记忆如果有链接指向的文件，会检查文件的修改时间并注入相应的过期警告。模型在后续所有对话里都能看到积累的记忆和新鲜度提示。
...
CLAUDE.md 是注入到每次对话系统提示中的项目指南。从工作目录向上递归加载多层 CLAUDE.md（项目级、用户级），内容合并后注入系统提示。

### 16.2 /init 创建

在项目目录下输入 `/init`，模型会自动：
1. 调查项目结构（语言、框架、包管理器）
2. 读关键文件（pyproject.toml、README.md 等）
3. 创建 CLAUDE.md，写项目特定的命令、架构、约定、陷阱

如果已有 CLAUDE.md，模型会建议改进而非覆盖。

### 16.3 自动维护

CLAUDE.md 的维护对用户完全无感：

- **主 Agent**：系统提示告诉模型 "如果发现新约定、命令或陷阱，用 edit_file 默默更新 CLAUDE.md，不需要特别告知用户"
- **记忆提取子 Agent**：每轮结束后后台检查 CLAUDE.md 是否需要更新，和记忆文件一起维护
- **内容原则**：只写 Claude 不知道会出错的内容——不写 README 已有的、不写从项目文件能看出来的、不写通用开发建议
