...
Engine 最初是一个 800 行的大类，包含 API 重试、工具编排、卡住检测、陈旧读取检测、持久化等所有逻辑。现在拆分为 4 个模块：
...
阈值：`上下文窗口 - 缓冲（13K）`，约为 93.5%。不是之前的 80%。

模型把自己看到的"旧消息"生成一份 **6 段式结构化摘要**（User Goal、Files & Project、Commands & Results、Errors & Fixes、Current State、Project Structure），远比一句话"Summarize the conversation"内容丰富。
