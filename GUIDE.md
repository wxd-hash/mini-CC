...
跨会话的持久记忆——让 minicc 在不同对话中"记住"关键信息。

### 12.1 三层记忆

| 层级 | 位置 | 内容 |
|------|------|------|
| daily_log.md | 记忆目录 | 每次对话中 `<memory>` 标签的内容，按时间追加 |
| MEMORY.md | 记忆目录 | 聚合后的记忆索引，加载到系统提示中 |
| dream consolidation | 自动运行 | 定期把 daily_log 合并成更新的 MEMORY.md |

### 12.2 Dream Consolidation

触发条件：距离上次 consolidation 超过 24 小时 **且** 至少有 5 个新会话。

触发后：
1. 加锁（防止多个实例同时执行）
2. 读取 daily_log.md + 最近会话的 transcript
3. 发给模型，让它生成更新后的 MEMORY.md
4. 写回 MEMORY.md，解锁

### 12.3 记忆内容举例

MEMORY.md 中可能包含：
- 用户偏好：用 pytest 不用 unittest，日志用 logging 不用 print
- 架构决策：分层架构 router → service → repository
- 常见陷阱：Windows 路径用反斜杠，跨平台代码要用 Path
